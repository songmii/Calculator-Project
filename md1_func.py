...
def log(calc: str) -> str:
    # log(2)(3)
    # log(base)(x)
    # calc = 'log(ln(~))' -> 이런 경우는 없는 걸로
    calc = calc.replace(')', " ").replace("(", " ")     # 괄호를 공백으로 바꿔서 split 사용할 예정
    calc = calc.split()
    # calc = ['log', '2', '3']
    base = calc[1]    # base = calc_mode1() -> 필요없을듯
    x = calc[2]     # x = calc_mode1() -> 필요없을듯 22
...
def nPr(calc):
    ...
    if type(n) is int and type(r) is int:
        ans = 1
        for i in range(r):
            ans *= n
            n -= 1
